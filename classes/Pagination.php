<?php
/*
   * Script name: phpSCG v1.0
   * Author: Gecko Systems (Bozhidar Slaveykov)
...
class Pagination {
    ...
    public function render($return_output = false) {
        ...
        $output = '<div class="pagi_pagi"><ul class="pagination">';
    ...
    public function variable_name($variable_name) {
        ...
        public function site_link($site_link) {

        // set the variable name
        $this->_properties['site_link'] = strtolower($site_link);
    }
    ...
    private function _build_uri($page) {
        ...
        if ($this->_properties['method'] == 'url') {
            ...
            if (preg_match('/\b' . $this->_properties['variable_name'] . '([0-9]+)\b/i', $this->_properties['base_url'], $matches) > 0) {
                ...
                $url = rtrim($this->_properties['site_link'], '/') . '/' . ($this->_properties['variable_name'] .'/'. $page);
